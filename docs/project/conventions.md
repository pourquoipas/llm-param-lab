...
- Run override (J4) → POST /api/suites/{id}/run body String (vuoto→null; JSON invalido→400) → {judgeId?,topK?}; runSuite risolve override PRIMA di findActive (judge ignoto→404); evaluate(overrideJudge) vince su expectedOutput E judge di suite; topK sopra temp/topP/seed del judge (judge senza topK salvabile, null→default modello); overload no-override (null) per chi c'è già
