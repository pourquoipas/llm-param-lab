...
- [x] `POST /api/admin/test-case` → ensure default model exists (active, from `LlmConfig`) + create a suite "code reviewer": shared code-review user prompt (non-thread-safe `SimpleCache`, 10k req/s) × 2 system personas ("system thinker", "Audit protocol") and a temperature sweep `[0.1, 1.0]`; judge = default model. (Suite content swapped on request: was "Agent smoke test" with 3 cases × `[0.5, 0.8]`.)
...
**Verify (done):** `node --check app.js` OK. Fresh boot → both buttons present; POST test-case → 201 + "code reviewer" (2 cases, temp [0.1,1.0], default model active); POST clean → 204 + models/suites `[]`.
...
| I4a | Seed sweep (separate from param sweep) + per-seed grouping | ✅ done | Suite: `seeds` (CLOB, JSON array) opzionale; ogni caso×combo gira per ogni seed (N seed = N run). Lista vuota → seed generato+persistato. `RunResult.seed` (migration 004). `runSuite`→`resolveSeeds`/`runOne(...,seed,...)`/`saveResult(...,seed)`. Summary raggruppa per seed (`RunSummaryResponse.seeds`→`SeedSummary`). UI: campo "Seed Sweep" + col Seed tabella + summary per seed. Seed passato al modello (Ollama/OpenAI). Test: `TestRunnerServiceSeedTest`, `TestRunnerServiceResilienceTest`/`Isolation` (runOne sig), `TestSuiteResourceTest#seedsRoundTrip`, `ResultResourceTest` (summary per seed). |
| I4b | Thinking stats (thinking tokens/time) + input/output t/s | ✅ done | `RunResult` +mig 005: `reasoning_tokens`(INT), `input_tps`/`output_tps`(DOUBLE). `saveResult` legge `OpenAiTokenUsage.outputTokensDetails().reasoningTokens()` (solo OpenAI-compatible, altrimenti null) e `throughput(tokens,latencyMs)` (tokens÷ms, null se latency/tokens sconosciuti). UI: colonne Thinking + In/Out t/s. **Limitazione documentata:** langchain4j 1.0.1 non espone il *tempo* di thinking (solo conteggi token + latenza totale). Test: `TestRunnerServiceTokenStatsTest` (OpenAI reasoning=25, base usage null, throughput 100/2000ms=50). |
| I5 | No default token limit (use model default unless in suite) | ✅ done | Già garantito dal codice: `buildParameters` setta `maxTokens` solo se sweepato (chiave `maxTokens` nel combo); altrimenti null → `applyCommon` non chiama `maxOutputTokens` → default del modello. Judge (linea judgeParameters) passa sempre null. Test: `TestRunnerServiceNoDefaultTokenCapTest` (no cap se non sweepato; cap=256 se sweepato). |
| I6 | Raw-JSON injection into chat request | ⚠️ non implementabile | langchain4j 1.0.1 serializza `ChatRequestParameters` a uno schema fisso: nessuna API `extra`/`raw`/`map` per iniettare JSON arbitrario (es. `"reasoning":{"effort":"low"}`). **Disponibile invece:** campo tipizzato `reasoningEffort(String)` (OpenAI-compatible), già cablato in `buildChatParams` — equivalente di `reasoning.effort`. Per i campi non tipizzati non c'è via in langchain4j → puntato ma non implementato. |
| I7 | README: interface, API + data-structure docs | ✅ done | README: sezioni "Interface (UI)" (4 tabs + top bar), "Patterns" (new-chat-per-task, provider-aware params, resilient run, seed sweep, idempotent seeding, schema versioning, offline test suite), API field reference (DTO shapes flat, `RunSummaryResponse` per-seed, `RunResult` fields) + curl example corretto. `docs/project/conventions.md` (≤40 righe, sweep/seed/token/validation) + `docs/project/base-classes.md` (RunSummaryResponse→SeedSummary, SuiteCreateRequest flat+seeds, RunResult seed/reasoningTokens/tps, TestSuite judge params+seeds). |
...
| R2 | REST delete endpoints (single, set, suite, all) | ✅ done |
